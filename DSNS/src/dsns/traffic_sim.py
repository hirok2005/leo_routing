...
class PoissonTrafficActor(Actor):

    def __init__(self, source: SatID, destination: SatID, avg_message_interval: float, message_size: int = 1500, update_interval: float = 60, reliable_messages: bool = False):
        super().__init__()
        self.source = source
        self.destination = destination
        self.avg_message_interval = avg_message_interval
        self.message_size = message_size
        self.update_interval = update_interval
        self.reliable_messages = reliable_messages
        
        self.next_message_time = 0.0
        self.current_batch_end = 0.0
        
        self.sampler = ExponentialSampler(scale=self.avg_message_interval)
        self.message_count = 0

    def generate_events(self) -> list[Event]:
        events = []
        self.current_batch_end += self.update_interval
        
        while self.next_message_time < self.current_batch_end:
            if not self.reliable_messages:
                message = DirectMessage(
                    source=self.source,
                    destination=self.destination,
                    data=f"Message {self.message_count}",
                ...
                message = HybridDirectMessage(
                    ...
                    data=f"Message {self.message_count}",
                    size=self.message_size,
                    reliable_data_size=self.message_size,
                    unreliable_data_size=0,
                )
                
            events.append(MessageCreatedEvent(
                time=self.next_message_time,
                message=message,
            ))
            
            self.message_count += 1

            self.next_message_time += self.sampler._sample() 

        events.append(GenerateMessagesEvent(time=self.current_batch_end))
        return events
...
class MultiPoissonTrafficActor(Actor):
    def __init__(self, message_config: list[tuple[str, SatID, SatID, int, float]], update_interval: float = 60, reliable_messages: bool = False, cutoff: Optional[int] = None):
        ...
        self.flows = []
        for name, source, dest, size, avg_interval in self.message_config:
            self.flows.append({
                "name": name,
                "source": source,
                "destination": dest,
                "size": size,
                "sampler": ExponentialSampler(scale=avg_interval),
                "next_message_time": 0.0,
                "message_count": 0
            })
    ...
    def generate_events(self) -> list[Event]:
        ...
        for flow in self.flows:
            while flow["next_message_time"] < self.current_batch_end and flow["next_message_time"] < self._cutoff:
                if not self.reliable_messages:
                    message = DirectMessage(
                        source=flow["source"],
                        destination=flow["destination"],
                        data=f"{flow['name']}-{flow['message_count']}",
                        size=int(flow["size"]),
                    )
                else:
                    message = HybridDirectMessage(
                        ...
                        data=f"{flow['name']}-{flow['message_count']}",
                        size=int(flow["size"]),
                        reliable_data_size=int(flow["size"]),
                        unreliable_data_size=0,
                    )
                    
                events.append(MessageCreatedEvent(
                    time=flow["next_message_time"],
                    message=message,
                ))
                
                flow["message_count"] += 1
                
                flow["next_message_time"] += flow["sampler"]._sample()

        if self.current_batch_end < self._cutoff:
            events.append(GenerateMessagesEvent(time=self.current_batch_end))
