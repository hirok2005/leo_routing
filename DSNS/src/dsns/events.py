...
class HardwareFailureEventUpdate(Event):
    """
    Event for a hardware failure update. Stores all currently failed links
    """
    
    def __init__(self, time: float, failed_links: set[tuple[int, int]], recovered_links: Optional[set[tuple[int, int]]] = None):
        super().__init__(time, ["failed_links"])
        self.failed_links = failed_links
        self.recovered_links = recovered_links
